#include "BloomExtractPipeline.h"

#include "../../core/HResult.h"

#include "BloomExtractPS.h"
#include "BloomExtractVS.h"
...
{
    BloomExtractPipeline::BloomExtractPipeline(
        ID3D12Device* device,
    ...
    void BloomExtractPipeline::CreateRootSignature(
        ID3D12Device* device)
    {
        ...
        core::ThrowIfFailed(
            serializationResult,
            "Failed to serialize "
            "bloom extraction root signature"
        );
        ...
        core::ThrowIfFailed(
            m_rootSignature->SetName(
                L"Gravity Forge "
                L"Bloom Extraction Root Signature"
            ),
            "Failed to name bloom extraction "
            "root signature"
        );
    }

    void BloomExtractPipeline::CreatePipelineState(
        ID3D12Device* device,
        DXGI_FORMAT renderTargetFormat)
    {
        ...
        renderTargetBlend.LogicOpEnable =
            FALSE;
        ...
        core::ThrowIfFailed(
            ...
            "Failed to create bloom extraction "
            "pipeline state"
        );
        ...
        core::ThrowIfFailed(
            m_pipelineState->SetName(
                L"Gravity Forge "
                L"Bloom Extraction PSO"
            ),
            "Failed to name bloom extraction PSO"
        );
    }
}